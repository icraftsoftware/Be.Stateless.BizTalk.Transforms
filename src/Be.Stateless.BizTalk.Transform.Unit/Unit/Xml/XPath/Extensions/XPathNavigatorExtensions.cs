...
namespace Be.Stateless.BizTalk.Unit.Xml.XPath.Extensions
{
	public static class XPathNavigatorExtensions
	{
		...
		internal static Func<XPathNavigator, ValuednessValidationCallback, ValuednessValidator> ValuednessValidatorFactory { get; set; }
			= (navigator, callback) => new(navigator, callback);
		...
		/// An optional validation callback that can be used to demote the <see cref="XmlSeverityType"/> so that no exception is
		/// thrown if all the empty elements or attributes are only to be considered as <see cref="XmlSeverityType.Warning"/>.
		/// </param>
		/// <returns>
		...
		/// </para>
		/// <para>
		/// Empty elements or attributes will be considered as an <see cref="XmlSeverityType.Error"/> condition which will entail
		/// that an <see cref="XmlException"/> will be thrown unless a <see cref="ValuednessValidationCallback"/> validation
		/// callback is passed to demote the severity of the error for a given element or attribute to <see
		/// cref="XmlSeverityType.Warning"/>.
		/// </para>
		/// </remarks>
